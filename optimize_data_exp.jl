...
using LinearAlgebra, Random, Distributions, Test, NLsolve
using DataFrames, XLSX, Missings
using NLopt, JuMP, Ipopt, Complementarity
...
myexpr = []
for j=1:D
    push!(myexpr[:,j], (1+g0)*(A'*p[:,j] .+ c .- x0[:,j]) .- g0.*data.p_bar)
end
